...
function BackArrow() {
  return (
    <svg width="10" height="18" viewBox="0 0 10 18" fill="none">
      <path d="M9 1L1 9L9 17" stroke="#32343E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function ChevronDownIcon() {
  return (
    <svg width="11" height="8" viewBox="0 0 11 8" fill="none">
      <path d="M1 1L5.5 6.5L10 1" stroke="#32343E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function PaymentMethodTab({ method, selected, onClick }) {
  ...
  return (
    <button onClick={() => onClick(method.id)} className="flex flex-col items-center gap-2 shrink-0" aria-pressed={selected}>
      <div className={`w-21.25 h-18 rounded-2xl flex items-center justify-center relative ${selected ? 'bg-white border-2 border-primary' : 'bg-[#F0F5FA]'}`}>
        <Icon />
        {selected && (
      ...
      <span className={`text-[13px] ${selected ? 'text-[#F58D1D] font-semibold' : 'text-[#A0A5BA]'}`}>
        {method.label}
      </span>
    </button>
  );
...
function CardIllustration() {
  return (
    <div className="relative w-52.5 h-35 rounded-[20px] overflow-hidden" style={{ background: 'linear-gradient(135deg, #F58D1D 0%, #F5A623 100%)', boxShadow: '0 16px 40px rgba(14,39,62,0.18)' }}>
      <div className="absolute inset-0" style={{ background: 'linear-gradient(145deg, transparent 33%, #D9534F 33%, #D9534F 62%, transparent 62%)', opacity: 0.9 }} />
      <div className="absolute top-3.5 left-3.5">
        <img src={mastercardImg} alt="Mastercard" className="w-10 h-7 object-contain" />
      </div>
      <div className="absolute bottom-5 right-8 w-10 h-10 bg-[#F7C948] rotate-45 opacity-90" />
      <div className="absolute bottom-0 right-0 w-16 h-16" style={{ background: 'linear-gradient(225deg, rgba(255,255,255,0.3) 0%, transparent 65%)' }} />
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      <div className="text-center">
        <p className="text-[#32343E] text-[16px] font-bold leading-normal mb-1">No {label.toLowerCase()} card added</p>
...
function CardRow({ card, selected, onSelect, onRemove, onEdit }) {
  ...
  return (
    <div
      className={`relative h-20.5 rounded-2xl bg-[#F0F5FA] px-5 flex flex-col justify-center cursor-pointer transition-all ${selected ? 'ring-2 ring-[#F58D1D]' : ''}`}
      onClick={() => onSelect(card.id)}
    >
      <p className="font-bold text-[#32343E] text-[16px] leading-normal mb-1.5">{label}</p>
      <div className="flex items-center gap-2">
        <div className="w-7 h-4.5 flex items-center shrink-0">
          {card.type === 'visa' ? <VisaIcon /> : <MastercardIcon />}
        </div>
        <span className="text-[#32343E]/50 text-[16px] leading-none">*************</span>
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3">
        ...
        >
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
            <path d="M1 1L9 9M9 1L1 9" stroke="#E53935" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </button>
        <button onClick={e => { e.stopPropagation(); onEdit(card); }} aria-label="Edit card">
          <ChevronDownIcon />
        </button>
      </div>
    </div>
...
export default function PaymentMethodPage() {
  ...
  return (
    <div className="flex flex-col h-svh bg-white">
      {/* ── Header ── */}
      <div className="flex items-center gap-4.5 px-6 pt-6 pb-2 shrink-0">
        <button
          onClick={() => navigate(-1)}
        ...
        <span className="text-[#181C2E] text-[17px]">Payment Method</span>
      </div>

      {/* ── Scrollable content ── */}
      <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar px-6 flex flex-col gap-6 pb-8">
        {/* Method tabs */}
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-1 pt-6">
          {METHODS.map(m => (
            <PaymentMethodTab key={m.id} method={m} selected={selectedMethod === m.id} onClick={setSelectedMethod} />
          ))}
        </div>

        {/* Cash info or empty card state */}
        {(selectedMethod === 'cash' || activeCards.length === 0) && (
          <div className="bg-[#F0F5FA] rounded-3xl p-6 flex items-center justify-center min-h-65">
            {selectedMethod === 'cash' ? (
              <div className="text-center">
                <div className="flex items-center justify-center mb-4"><CashIcon /></div>
                <p className="text-[#181C2E] text-[18px] font-bold mt-4 mb-1">Pay with Cash</p>
                <p className="text-[#A0A5BA] text-[14px]">Pay the delivery person on arrival</p>
              </div>
            ) : (
        ...
        {/* Add new — only for card methods */}
        {selectedMethod !== 'cash' && (
          <button
            onClick={() => navigate('/add-card', { state: { type: selectedMethod } })}
          ...
          >
            ...
            <span className="text-primary text-[14px] font-bold tracking-widest uppercase">Add New</span>
          </button>
        )}
      </div>
    </div>
